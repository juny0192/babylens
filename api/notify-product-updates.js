    ...
    for (const userId of Object.keys(userEmailMap)) {
      ...
      const htmlBody = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
          <h2 style="color: #9EC4C5; margin-bottom: 4px;">MumSaid</h2>
          ...
          <p style="font-size: 14px; color: #374151;">
            Check out the latest details on <a href="https://mumsaid.vercel.app" style="color: #9EC4C5; text-decoration: none; font-weight: 600;">MumSaid</a>.
          </p>
          <hr style="border: none; border-top: 1px solid #f3f4f6; margin: 24px 0 12px;" />
          <p style="font-size: 11px; color: #9ca3af;">
            You're receiving this because you enabled notifications in MumSaid Settings.
      ...
      try {
        const sendRes = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: 'MumSaid <onboarding@resend.dev>',
            to: email,
            subject: `MumSaid: ${products.length} saved product${products.length > 1 ? 's' : ''} updated`,
            html: htmlBody,
          }),
        })

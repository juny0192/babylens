...
export default function Navbar() {
  ...
  const tabs = [
    ...
    ...(user ? [{ to: '/saved', label: 'Saved', icon: HeartIcon, badge: true }] : []),
    user
      ? { to: '/account', label: 'Account', icon: AccountIcon }
      : { to: '/auth', label: 'Log In', icon: LoginIcon },
  ]
    ...
    <>
      ...
      <header className="fixed top-0 inset-x-0 z-40 bg-white border-b border-gray-100 h-14 flex items-center px-4">
        <button onClick={() => navigate('/')} className="flex items-center gap-2">
          ...
          <span className="text-xs bg-brand-100 text-brand-600 px-2 py-0.5 rounded-full font-medium">beta</span>
        </button>
      </header>
